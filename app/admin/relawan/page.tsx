...
import { Search, Plus, Edit2, Trash2, ChevronDown, CheckCircle, Clock, AlertCircle } from 'lucide-react';
import { useRelawanStore, Relawan, Divisi, StatusRelawan } from '@/lib/stores';
...
export default function RelawanPage() {
  const { relawanList, addRelawan, updateRelawan, deleteRelawan } = useRelawanStore();
  const [search, setSearch] = useState('');
  const [filterStatus, setFilterStatus] = useState('');
  const [showModal, setShowModal] = useState(false);
  const [editTarget, setEditTarget] = useState<Relawan | null>(null);

  const filtered = relawanList.filter((r) => {
    const matchesSearch = r.namaLengkap.toLowerCase().includes(search.toLowerCase()) ||
  ...
  const generateNewIdRelawan = () => {
    ...
    const usedNumbers = relawanList
      .map(r => parseInt(r.idRelawan.replace('SPPG-', ''), 10))
      .filter(n => !isNaN(n))
      .sort((a, b) => a - b);
  ...
  const handleSave = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const data: Partial<Relawan> = {
      namaLengkap: fd.get('namaLengkap') as string,
      email: fd.get('email') as string,
      noTelepon: fd.get('noTelepon') as string,
      nik: fd.get('nik') as string,
      divisi: fd.get('divisi') as Divisi,
      status: fd.get('status') as StatusRelawan,
    };

    if (editTarget) {
      updateRelawan(editTarget.id, data);
    } else {
      addRelawan({
        id: Date.now(),
        idRelawan: fd.get('idRelawan') as string,
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">{relawanList.length} relawan terdaftar</p>
        </div>
        <button
                ...
                <tr key={r.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-4 py-3.5">
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="text-slate-700 font-medium">{r.namaLengkap}</span>
                    </div>
                  </td>
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl animate-scale-in">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="flex gap-3 mt-6 pt-2">
                ...
                <button
                  type="submit"
                  className="flex-1 py-2.5 rounded-xl bg-emerald-500 text-white text-sm
